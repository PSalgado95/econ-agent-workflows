...
If the skill changed source files in the `econ-agent-workflows` package itself, remember that installed runtime copies are not updated until `python install.py --force` is run from the source checkout and Claude Code is restarted.
