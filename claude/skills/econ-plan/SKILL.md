...
- Use `CLAUDE.md`/`README.md` for orientation only, never as the research plan. If no backbone exists and future sessions will need the context, ask whether to create a compact `PROJECT_BRIEF.md` (not for narrow one-offs). A new project's `DEFINITIONS.md` can be seeded from the backbone's definitions section or a graduated scope memo.
