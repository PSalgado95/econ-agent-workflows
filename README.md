...
`econ-review` is built around research-audit lenses. The review checks the main places where research workflows can go wrong: data provenance, sample and variable construction, specification choice, inference, robustness, output consistency, and reproducibility. Different reviewer agents specialise in these questions, and the parent review skill assigns the relevant ones for the task. A data-cleaning review focuses on provenance, transformations, sample rules, and output consistency. A results review adds specification, estimation, inference, robustness, or dynamics checks when the estimates require them.

Reviewer lenses live as custom agents under `.codex/agents/`, with shared rules in [references/reviewer-protocol.md](references/reviewer-protocol.md). If the agents are not installed or subagent tools are unavailable, `econ-review` reports that the panel did not fully run. Promotion-grade reviews stop or ask before continuing in a degraded mode.
...
`econ-review` depends on three pieces: the skill text, the reviewer agents, and the shared reviewer protocol. Installing only the skills leaves the review panel incomplete.
...
from the repo root. Confirm it installs the core skills, auxiliary helper skills, reviewer agents, and shared protocol.
```
